...
export async function GET(req: NextRequest) {
  try {
    ...
    // Monthly case volumes (last 6 months) - Optimized to run in parallel
    ...
    // Technician workload - Optimized by using groupBy and in-memory aggregation
    const [allTechnicians, caseCounts] = await Promise.all([
    ...
    const techWorkload = allTechnicians.map((tech) => {
      const techCounts = caseCounts.filter((c) => c.technicianId === tech.id);
