...
export async function GET(_req: NextRequest) {
  try {
    ...
    // Prepare monthly queries for parallel execution
    ...
    const topDentistData = topDentistsRaw.map((d) => {
      const revData = dentistRevenueRaw.find((r) => r.dentistId === d.id);
      return {
        id: d.id,
        name: d.name,
        clinicName: d.clinicName,
        caseCount: d._count.cases,
        revenue: revData?._sum.amount || 0,
      };
    });

    // Aggregate technician workload in memory
