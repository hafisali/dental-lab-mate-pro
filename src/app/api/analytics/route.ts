...
export async function GET() {
  try {
    ...
    // ⚡ Bolt Optimization: Parallelize all independent database queries to reduce overall response time.
    // This replaces a sequence of 'await' calls with a single Promise.all block.
    const [
      overdueCasesRaw,
      dueSoonCasesRaw,
      casesByStatus,
      casesByWorkType,
      deliveredCasesCombined,
      monthlyCaseVolumes,
      topDentistsRaw,
      allTechnicians,
      techStatsRaw,
      ...
      // 5. Combined delivered cases for metrics (turnaround & on-time rate)
      // Optimization: Combined two separate 'DELIVERED' status queries into one.
      prisma.case.findMany({
        where: { labId, status: "DELIVERED" },
        select: { createdAt: true, updatedAt: true, dueDate: true },
      }),
      // 6. Monthly volumes (last 6 months)
      // Optimization: Refactored sequential loop counts into parallelized promises.
      Promise.all(
      ...
      // 9. Technician status stats
      // Optimization: Replaces N*2 count queries with a single groupBy query.
      (prisma.case.groupBy({
    ...
    // Post-processing logic to format data for response
    ...
    // Turnaround and on-time rate calculation
    let avgTurnaround = 0;
    let onTimeRate = 0;
    if (deliveredCasesCombined.length > 0) {
      const totalDays = deliveredCasesCombined.reduce((sum, c) => {
        const diff = new Date(c.updatedAt).getTime() - new Date(c.createdAt).getTime();
        return sum + diff / (1000 * 60 * 60 * 24);
      }, 0);
      avgTurnaround = Math.round((totalDays / deliveredCasesCombined.length) * 10) / 10;

      const casesWithDue = deliveredCasesCombined.filter(c => c.dueDate);
    ...
    // Tech workload aggregation via Map for O(N) correlation
