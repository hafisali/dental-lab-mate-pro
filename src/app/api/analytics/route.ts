...
export async function GET() {
  try {
    ...
    // Optimized: Parallel execution of all independent queries
    const [
      overdueCases,
      dueSoonCases,
      casesByStatus,
    ...
    // Second stage of parallel queries (dependent on results from stage 1)
    const [dentistRevenue, caseCountsByTech] = await Promise.all([
      prisma.case.groupBy({
        by: ["dentistId"],
        where: { dentistId: { in: topDentists.map((d) => d.id) } },
        _sum: { amount: true },
      }),
      prisma.case.groupBy({
        by: ["technicianId", "status"],
        where: { labId, technicianId: { in: allTechnicians.map((t) => t.id) } },
        _count: { id: true },
      })
    ]);
    ...
    const topDentistData = topDentists.map((d) => ({
      ...
      revenue: dentistRevenue.find((r) => r.dentistId === d.id)?._sum.amount || 0,
    }));

    const techWorkload = allTechnicians.map((tech) => {
      const techCases = caseCountsByTech.filter((c) => c.technicianId === tech.id);
