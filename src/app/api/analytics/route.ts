...
export async function GET() {
  try {
    ...
    const [
      ...
      // 9. Top dentist revenues (optimized aggregation)
      prisma.case.groupBy({
        by: ["dentistId"],
        _sum: { amount: true },
        where: { labId },
      }) as unknown as Promise<DentistRevenueCount[]>,
      // 10. Technician list
      prisma.user.findMany({
        where: { labId, role: "TECHNICIAN", active: true },
        select: { id: true, name: true },
      }),
      // 11. Technician workload (optimized aggregation)
      prisma.case.groupBy({
    ...
    const revenueMap = new Map(topDentistRevenues.map((r) => [r.dentistId, r._sum.amount || 0]));
