...
export async function GET() {
  try {
    ...
    // Monthly revenue (last 6 months) - Optimized by parallelizing database calls using Promise.all to reduce roundtrips from 6 down to 1.
    ...
    const monthlyPaymentsResults = await Promise.all(monthsData.map((m) => m.promise));

    const monthlyRevenue = monthsData.map((m, idx) => {
      const result = monthlyPaymentsResults[idx] as { _sum: { amount: number | null } };
      return {
        month: m.monthLabel,
