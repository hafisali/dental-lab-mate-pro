...
export async function GET() {
  try {
    ...
    // Pre-calculate date ranges for last 6 months
    const monthlyRanges = Array.from({ length: 6 }, (_, i) => {
      const d = new Date();
      d.setDate(1); // Set to 1st to avoid month-skipping logic error on 31st
    ...
    const [
      ...
      // Parallelize monthly revenue queries (reduces 6 sequential DB round-trips)
    ...
    // Map monthly revenue results from Promise.all
