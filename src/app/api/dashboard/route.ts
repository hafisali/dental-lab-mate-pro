...
export async function GET() {
  try {
    ...
    // Monthly revenue (last 6 months) - Optimized: Using in-memory aggregation to avoid N+1 queries
    const monthlyRevenue: { month: string; revenue: number }[] = [];
    for (let i = 5; i >= 0; i--) {
      const d = new Date();
      // Ensure we don't have side effects from setMonth across loop iterations
