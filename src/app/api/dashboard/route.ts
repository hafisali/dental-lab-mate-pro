...
export async function GET() {
  try {
    ...
    // Prepare monthly revenue queries for parallel execution
    const months: string[] = [];
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      months.push(startOfMonth.toLocaleDateString("en-US", { month: "short", year: "2-digit" }));
    ...
    // Format monthly revenue results
    const monthlyRevenue = monthlyResults.map((result, index) => ({
      month: months[index],
      revenue: (result as { _sum: { amount: number | null } })._sum.amount || 0,
    }));
