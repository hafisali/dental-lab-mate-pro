...
export async function GET() {
  try {
    ...
    // Monthly revenue (last 6 months) - OPTIMIZED: Run queries in parallel via Promise.all to avoid 6 sequential database roundtrips (O(N) down to O(1) sequential calls)
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const promise = prisma.payment.aggregate({
        where: {
          dentist: { ...tenantWhere },
      ...
      monthlyRevenuePromises.push(promise);
    }
