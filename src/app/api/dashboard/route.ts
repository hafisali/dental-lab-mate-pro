...
export async function GET() {
  try {
    ...
    // Prepare date ranges for the last 6 months to parallelize revenue queries
    const monthlyRanges = Array.from({ length: 6 }, (_, i) => {
      const d = new Date();
      // Set to 1st of month to avoid issues when current day is 31st and target month has fewer days
      d.setDate(1);
    ...
    // ⚡ Bolt Optimization: Parallelize all 13 database queries to reduce overall latency.
    // This includes summary stats AND the 6-month revenue breakdown previously in a sequential loop.
    const [
      todayCases,
      pendingCases,
      deliveredCases,
      ...
      // Spread the 6 monthly revenue queries
    ...
    // Construct monthly revenue array from parallelized results
    const monthlyRevenue = monthlyRanges.map((range, index) => ({
      month: range.startOfMonth.toLocaleDateString("en-US", {
        month: "short",
        year: "2-digit",
      }),
      revenue: (monthlyPaymentsResults[index] as unknown as MonthlyPaymentResult)._sum.amount || 0,
    }));
